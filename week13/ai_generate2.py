...
df = pd.read_csv('set2_sampled.csv')
df['source_leaning'] = df['source'].map(source_leanings)
df['clean'] = 'HEADLINE: ' + df['title'] + '\n\n' + 'STORY TEXT: ' + df['text']
df['gpt_leaning'].fillna('not set', inplace=True)
df_others = df[df['gpt_leaning'] != 'not set']
df2 = df[df['gpt_leaning'] == 'not set']
df2['gpt_leaning'] = None
df = df2.copy()
...
result_df.to_csv('set2_sampled_withgpt4o.csv', index=False)
