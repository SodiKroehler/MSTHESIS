...
def standardize_rating_df(df):
    ...
    df['user_poli_leaning_num'] = df["user_poli_leaning"].map(poli_leaning_dict).astype(float)
    df['user_agree_with'] = df['user_agree_with'].astype(bool) #just t/f
    df['user_article_subject'] = df['user_article_subject'].astype(str) #missing ai reg here, but otherwise the same
    df['user_wants_to_see_num'] = df['user_wants_to_see'].map(want_to_see_dict).astype(float)
    df['user_ai_leaning_num'] = df['user_ai_leaning'].map(ai_leaning_dict).astype(float)
    df['user_aireg_leaning_num'] = df['user_aireg_leaning'].map(ai_reg_leaning_dict).astype(float)
    df['user_imm_leaning_num'] = df['user_imm_leaning'].map(imm_leaning_dict).astype(float)
    df['user_bias_amnt_num'] = df['user_bias_amnt'].map(bias_amount_dict).astype(float)
    df['user_bias_cause'] = df['user_bias_cause'].astype(str).fillna('')  #also just leaving the same here too
    df[gpt_leaning_name] = df['gpt_leaning'].map(poli_leaning_dict).astype(float)
    df['source_leaning'] = df['source'].map(source_leaning_dict).astype(str).fillna('')
    df['source_leaning'] = df['source_leaning'].map(poli_leaning_dict).astype(float)
    df['rater'] = df['rater_email'].map(rater_dict).astype(str).fillna('')
    ...
    # print(df['user_poli_leaning'].value_counts())
    # print(df['user_poli_leaning'].isna().sum())
    # print(df.shape)
    # def isCoded(row):
    #     if row['user_poli_leaning_num'] is not None:
    #         return True
    # df['coded'] = df[columns_to_check_for_coding].notna().any(axis=1)
    # df['coded'] = df.apply(isCoded, axis=1)
    # df = df[df['coded'] == True]  # Only keep coded articles
    df=df.dropna(subset=['user_poli_leaning_num'])
    df['coded'] = True
    ...
    #         df.loc[df['source_url'] == row.source_url, f'user_poli_leaning_num_{rater}'] = row.user_poli_leaning_num
    #         df.loc[df['source_url'] == row.source_url, f'user_agree_with_{rater}'] = row.user_agree_with
    #         df.loc[df['source_url'] == row.source_url, f'user_article_subject_{rater}'] = row.user_article_subject
    #         df.loc[df['source_url'] == row.source_url, f'user_wants_to_see_num_{rater}'] = row.user_wants_to_see_num
    #         df.loc[df['source_url'] == row.source_url, f'user_ai_leaning_num_{rater}'] = row.user_ai_leaning_num
    #         df.loc[df['source_url'] == row.source_url, f'user_aireg_leaning_num_{rater}'] = row.user_aireg_leaning_num
    #         df.loc[df['source_url'] == row.source_url, f'user_imm_leaning_num_{rater}'] = row.user_imm_leaning_num
    #         df.loc[df['source_url'] == row.source_url, f'user_bias_amnt_num_{rater}'] = row.user_bias_amnt_num
    #         df.loc[df['source_url'] == row.source_url, f'user_bias_cause_{rater}'] = row.user_bias_cause

    abomindable_dictionary = {}
    for arti in df['source_url'].unique():
    ...
    #then we add the abominable dictionary to the df
    for arti, values in abomindable_dictionary.items():
        for key, value in values.items():
            df.loc[df['source_url'] == arti, key] = value
...
#2 and 3 i think are the same set
...
flipped_want_to_see_dict = want_to_see_dict.copy()
flipped_ai_leaning_dict = ai_leaning_dict.copy()
flipped_imm_leaning_dict = ai_reg_leaning_dict.copy()

for key in ["THE_SAME", "SIMILARLY", "DIFFERENTLY", "WOULD_NOT_HAVE_WRITTEN_ABOUT_IT", "VERY_DIFFERENTLY"]:
    flipped_want_to_see_dict.pop(key, None)

for key in ["PRO_AI", "ANTI_AI"]:
    flipped_ai_leaning_dict.pop(key, None)

for key in ["PRO_IMMIGRATION", "ANTI_IMMIGRATION"]:
    flipped_imm_leaning_dict.pop(key, None)

flipped_poli_leaning_dict = {v: k for k, v in small_poli_leaning_dict.items() if v is not None}
flipped_want_to_see_dict = {v: k for k, v in flipped_want_to_see_dict.items() if v is not None}
flipped_ai_leaning_dict = {v: k for k, v in flipped_ai_leaning_dict.items() if v is not None}
flipped_ai_reg_leaning_dict = {v: k for k, v in ai_reg_leaning_dict.items() if v is not None}
flipped_imm_leaning_dict = {v: k for k, v in flipped_imm_leaning_dict.items() if v is not None}
flipped_bias_amount_dict = {v: k for k, v in bias_amount_dict.items() if v is not None}
# Remove unwanted keys from flipped_poli_leaning_dict


# for key in ["sodi.kroehler@gmail.com", "emmetmathieu@gmail.com"]:
#     flipped_want_to_see_dict.pop(key, None)
...
long_df = long_df[[
    ...
    "user_bias_amnt", "user_bias_cause", "set3_ID", "coded"
]]
